...
INITIAL_SKILLS = {
    "math_reasoning": ROOT / "algorithms" / "trace2skill-settings" / "skills" / "math_reasoning",
    "docvqa": ROOT / "algorithms" / "trace2skill-settings" / "skills" / "docvqa",
}
...
def main() -> None:
    ...
    if args.trajectory_manifest:
        ...
        cmd = [
            sys.executable,
            str(ROOT / "algorithms" / "trace2skill-settings" / "scripts" / "aggregate_analysis_records.py"),
            "--manifest",
    ...
    if len(generated_skill.read_text(encoding="utf-8").splitlines()) > args.max_skill_lines:
        ...
        cmd = [
            sys.executable,
            str(ROOT / "algorithms" / "trace2skill-settings" / "scripts" / "compress_skill_markdown.py"),
            "--input",
            str(pre_compression),
            "--output",
            str(generated_skill),
