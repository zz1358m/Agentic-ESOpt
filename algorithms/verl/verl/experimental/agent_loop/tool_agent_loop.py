...
from algorithms.verl_trace2skill.math_protocol import is_first_bash_action_only
...
@register("paper_react_cli_agent")
@register("tool_agent")
class ToolAgentLoop(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        ...
        cls.max_turn_response_length = config.actor_rollout_ref.rollout.multi_turn.max_turn_response_length
        ...
        if cls.paper_react_cli:
            ...
            cls.tool_parser = ToolParser.get_tool_parser(cls.tool_format, cls.tokenizer)
        print(f"Initialized tools: {cls.tools}")

        cls.apply_chat_template_kwargs = config.data.get("apply_chat_template_kwargs", {})
        cls.use_inference_chat_template = config.actor_rollout_ref.rollout.multi_turn.use_inference_chat_template
    ...
    async def _handle_generating_state(
        ...
        # Qwen's closing tool-call tag is a dedicated token. Stop as soon as
        # the model emits it so the tool is executed immediately instead of
        # allowing the model to hallucinate later user/tool turns in the same
        ...
        if self.text_react:
            turn_sampling_params = paper_react_sampling_params(
                ...
                self.tokenizer,
                max_new_tokens=self.max_turn_response_length,
            )
        else:
            tool_call_end_id = self.tokenizer.convert_tokens_to_ids("</tool_call>")
            if isinstance(tool_call_end_id, int) and tool_call_end_id != self.tokenizer.unk_token_id:
                stop_token_ids = list(turn_sampling_params.get("stop_token_ids") or [])
                if tool_call_end_id not in stop_token_ids:
                    stop_token_ids.append(tool_call_end_id)
                turn_sampling_params["stop_token_ids"] = stop_token_ids
                turn_sampling_params["no_stop_trim"] = True
        ...
        # Keep the structured history synchronized with the authoritative
        # token history so every assistant/tool turn is retained.
        retained_response_ids = agent_data.response_ids
        if not ignore_termination and len(agent_data.response_mask) >= self.response_length:
            overflow = len(agent_data.response_mask) - self.response_length
            if overflow:
                retained_response_ids = retained_response_ids[:-overflow]
        assistant_message = await self.loop.run_in_executor(
            None, lambda: self.tokenizer.decode(retained_response_ids, skip_special_tokens=True)
        )
        ...
        if self.paper_react_cli:
            ...
            if decision.kind == "action":
                ...
                if action_error is not None:
                    ...
                    warning = observation_message(name, action_error)
                    if not await self._append_text_messages(
                        agent_data,
            ...
            if self.max_user_turns and agent_data.user_turns >= self.max_user_turns:
                return AgentState.TERMINATED
            warning = observation_message("format_check", decision.message or "")
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        if self.text_react:
            ...
            for response in responses:
                ...
                if not text.startswith("Observation from "):
                    text = observation_message("bash", text)
                add_messages.append({"role": "user", "content": text})
            if not await self._append_text_messages(agent_data, add_messages):
                return AgentState.TERMINATED
            return AgentState.GENERATING
        ...
        if self.processor is not None:
            ...
            if self.use_inference_chat_template:
                # Qwen3.5 rejects a standalone tool message because there is
                # no preceding user query. Render the incremental tool suffix
                # behind a disposable user prefix, then strip only that
                # prefix. Existing prompt_ids are never reset.
                stub_messages = [{"role": "user", "content": "__verl_incremental_context__"}]
                stub_ids, combined_ids = await asyncio.gather(
                    self.loop.run_in_executor(
                        None,
                        lambda: self.tokenizer.apply_chat_template(
                            stub_messages, add_generation_prompt=False, tokenize=True
                        ),
                    ),
                    self.loop.run_in_executor(
                        None,
                        lambda: self.tokenizer.apply_chat_template(
                            stub_messages + add_messages, add_generation_prompt=True, tokenize=True
                        ),
                    ),
                )
                if combined_ids[: len(stub_ids)] != stub_ids:
                    raise ValueError("Incremental inference chat template prefix mismatch")
                response_ids = combined_ids[len(stub_ids) :]
            else:
    ...
    async def _append_text_messages(self, agent_data: AgentData, add_messages: list[dict[str, Any]]) -> bool:
        agent_data.messages.extend(add_messages)
        if self.processor is not None:
            ...
            model_inputs = self.processor(text=[raw_response], images=None, return_tensors="pt")
            response_ids = model_inputs.pop("input_ids").squeeze(0).tolist()
        else:
            response_ids = await self.loop.run_in_executor(
                None,
