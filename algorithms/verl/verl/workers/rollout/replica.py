...
def get_rollout_replica_class(rollout: str) -> type[RolloutReplica]:
    if rollout == "vllm":
        ...
        try:
            ...
            # A failed vLLM import can leave partially initialized modules
            # behind (for example, when old vLLM re-registers a Transformers
            # config). The CPU-only driver only needs the lightweight mock.
