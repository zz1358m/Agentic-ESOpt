...
from algorithms.verl_trace2skill.local_bash_tool import LocalBashTool
...
class MathBashToolTests(unittest.IsolatedAsyncioTestCase):
    async def test_math_config_executes_without_an_image(self) -> None:
        declaration = yaml.safe_load(
            (ROOT / "algorithms/verl_trace2skill/math_bash_tool_config.yaml").read_text(encoding="utf-8")
        )["tools"][0]
        tool = LocalBashTool(
            declaration["config"],
